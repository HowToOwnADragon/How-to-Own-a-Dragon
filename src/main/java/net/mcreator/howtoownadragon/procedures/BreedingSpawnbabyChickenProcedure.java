...
public class BreedingSpawnbabyChickenProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (Mth.nextInt(RandomSource.create(), 1, 2) == 1) {
			...
			}.compareDistOf(x, y, z)).findFirst().orElse(null)).getPersistentData().putString("chickencolor",
					(((Entity) world.getEntitiesOfClass(ChickenEntity.class, AABB.ofSize(new Vec3(x, y, z), 10, 10, 10), e -> true).stream().sorted(new Object() {
						Comparator<Entity> compareDistOf(double _x, double _y, double _z) {
							return Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_x, _y, _z));
						}
					}.compareDistOf(x, y, z)).findFirst().orElse(null)).getPersistentData().getString("chickencolor")));
		} else {
