...
public class SpawnBabyNadderProcedure {
			...
			HowToOwnADragonMod.queueServerWork(1, () -> {
				{
					...
					for (Entity entityiterator : _entfound) {
						if (entityiterator instanceof NadderFemaleEntity && entityiterator.getPersistentData().getDouble("Age") <= 639) {
			...
			HowToOwnADragonMod.queueServerWork(1, () -> {
				{
					...
					List<Entity> _entfound = world.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate(4 / 2d), e -> true).stream().sorted(Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_center)))
							.collect(Collectors.toList());
					for (Entity entityiterator : _entfound) {
						if (entityiterator instanceof NadderMaleEntity && entityiterator.getPersistentData().getDouble("Age") <= 639) {
							entityiterator.getPersistentData().putDouble("Age", 100);
						}
					}
				}
