...
public class JuvenileStoneGrowUpMaleGronckleProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		if (entity == null || sourceentity == null)
			return;
		if ((entity.getPersistentData().getString("groncklecolor")).equals("pink") && entity instanceof JuvenileGronckleMaleEntity) {
			if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == Blocks.STONE.asItem()) {
				...
				JuvenileGronckleGrowUpProcedureProcedure.execute(world, x, y, z, entity);
				...
				}.compareDistOf(x, y, z)).findFirst().orElse(null)).getPersistentData().putString("gronklecolor", "pink");
