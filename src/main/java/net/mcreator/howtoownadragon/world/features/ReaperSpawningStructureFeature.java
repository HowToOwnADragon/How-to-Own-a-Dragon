...
import net.mcreator.howtoownadragon.procedures.ReaperSpawningStructureOnStructureInstanceGeneratedProcedure;

import java.util.Set;

public class ReaperSpawningStructureFeature extends Feature<NoneFeatureConfiguration> {
	private final Set<ResourceKey<Level>> generate_dimensions = Set.of(Level.OVERWORLD);
	private StructureTemplate template = null;

	public ReaperSpawningStructureFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("how_to_own_a_dragon", "empty_reaper"));
		if (template == null)
			return false;
		...
		if ((context.random().nextInt(1000000) + 1) <= 1000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (template.placeInWorld(context.level(), spawnTo, spawnTo, new StructurePlaceSettings().setMirror(Mirror.values()[context.random().nextInt(2)]).setRotation(Rotation.values()[context.random().nextInt(3)]).setRandom(context.random())
					...
					ReaperSpawningStructureOnStructureInstanceGeneratedProcedure.execute(world, x, y, z);
					anyPlaced = true;
				}
			}
		}
